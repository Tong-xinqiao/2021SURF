...
for i in range(10):
    ...
    count = 0
    top1000 = map(difference.index, heapq.nlargest(3000, difference))
    for i in top1000:
        X_f_train_2 = np.concatenate((X_f_train_2, [newLHS[i]]), axis=0)
        if (0.02 <= newLHS[i][0] <= 0.82 and 0.8 <= newLHS[i][1] <= 0.81) or (
                0.05 <= newLHS[i][0] <= 0.06 and 0.1 <= newLHS[i][1] <= 0.9):
            count = count + 1
            X_f_train = np.concatenate(([newLHS[i]], X_f_train), axis=0)
        else:
            X_f_train = np.concatenate((X_f_train, [newLHS[i]]), axis=0)
    separation = separation + count

np.savetxt("adaptiveLHS500Sample_4w_LargeErrorInSteps.csv", X_f_train_2, delimiter=',')
# ######################################################################
# application
model = PhysicsInformedNN(X_u_train, u_train, X_f_train, layers, lb, ub, separation)
model.train()
...
np.savetxt("adaptiveLHS500_4w.csv", u_pred, delimiter=',')

error_u = np.linalg.norm(u_star - u_pred, 2) / np.linalg.norm(u_star, 2)
print('Error u: %e' % (error_u))

# Training time: 24835.6984
